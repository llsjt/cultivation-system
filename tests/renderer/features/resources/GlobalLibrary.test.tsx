...
import type { GetGlobalResourcesOutput, GetHomeOverviewOutput, IpcResult, ResourceSummary } from '../../../../src/shared/dto';
import type { CultivationAPI } from '../../../../src/shared/types/api';
import { GlobalLibrary } from '../../../../src/renderer/features/resources/GlobalLibrary';
...
const globalResources = {
  items: [
    {
      ...
      project: { id: projects[0].id, name: projects[0].name, status: projects[0].status, progress_percent: projects[0].progress_percent },
    },
    {
      resource: algoExercise,
      project: { id: projects[1].id, name: projects[1].name, status: projects[1].status, progress_percent: projects[1].progress_percent },
    },
    {
      resource: algoBook,
      project: { id: projects[1].id, name: projects[1].name, status: projects[1].status, progress_percent: projects[1].progress_percent },
  ...
  total: 3,
} satisfies GetGlobalResourcesOutput;

function renderLibrary(overrides: Partial<ComponentProps<typeof GlobalLibrary>> = {}) {
  const api = {
    get_global_resources: vi.fn(() => Promise.resolve(ok(globalResources))),
